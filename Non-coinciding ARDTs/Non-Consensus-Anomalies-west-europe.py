...
flds = ['guan_waliser']#['cascade_bard_v1','guan_waliser','reid250','mundhenk_v2']
...
for vb,pts in zip(vbls,clim_data_path):
    ...
    for fx,f in enumerate(flds):
        ...
        ar_data = xr.open_mfdataset(glob.glob(f'../larger_region_AR_capture/{reg}-{f}/*.nc',recursive=True),parallel=True)#.sel(time=slice('1980','2018'))
