...
def cart_plot(axes):
    axes.add_feature(cf.BORDERS,linewidth=1)
    axes.add_feature(cf.COASTLINE,linewidth=1)
    gl=axes.gridlines(crs=ccrs.PlateCarree(), draw_labels=True, linewidth=1, color='gray', alpha=0.01, linestyle='--')
    axes.set_extent([-5,5,4,8])
