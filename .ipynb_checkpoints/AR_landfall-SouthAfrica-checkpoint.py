...
'''
#####./AR_landfall_sel-Copy2.py new_mundhenk/ larger_region_AR_capture 55 56 -4 -5 0.8 1
...
flds =['cascade_bard_v1/']#['/cascade_bard_v1/','/guan_waliser/','/reid250/']#,'mundhenk/'] #algorithm names for mundhenk use 'mundhenk/' 
...
for idx, dx in enumerate(fpath):
    ...
    print(file)
    yy = 1980 
#global/cscratch1/sd/kquagra/ARs Work/new_mundhenk/MERRA2.ar_tag.Mundhenk_v2.3hourly.19860101-19861231.nc4

    #iterate through the dataset filepaths 
    for ff in file:

        with xr.open_dataset(ff,chunks={'time':10,'lon':576,'lat':361})['ar_binary_tag'] as dd : 
            data = dd.isel(time=dd.time.dt.month.isin([6,7,8])) #laod data and select DJF season only 
        ...
        if len(Tsp) > 0 : 
            ...
            final_path = f'{save_path}{reg}-{flds[ct][1:]}'
            print(final_path)
            if os.path.isdir(final_path) == False:
                os.mkdir(final_path)
            print(f'Saving netcdf data to {final_path}') 
            concat_data.to_netcdf(f'{final_path}{flds[ct][:-1]}_{reg}-landfall_ARs__{th1}-{th2}_{yy}.nc') #save the data 
...
def mp_process(data_path,save_path,th1,th2,llon,rlon,blat,tlat):
    ...
    p = Pool() #select number of processes or leave the system to do so
    res  = p.map(func=reg_criteria_sel(data_path, save_path, th1, th2, llon, rlon, blat, tlat)) #call the processors and map over the function 
    p.close() 
    p.join()

if __name__ == "__main__":
    mp_process(data_path, save_path, th1, th2, llon, rlon, blat, tlat) #run the processes 
'''
 
    

####./AR_landfall_sel_2-Copy2.py '/global/cscratch1/sd/kquagra/ARs Work/new_' larger_region_AR_capture/ 0.8 1.0 7 8 60 61  for mdk


###./AR_landfall_sel_2-Copy2.py '/global/project/projectdirs/m1517/cascade/external_datasets/ARTMIP/tier1/ftp_mirror/' larger_region_AR_capture/ 0.8 1.0 7 8 60 61 for other alg use this 
## with guan and waliser, use chunks {'time':1} and add the lonlat chunks as well, whereas with the rest, do not specify time chunks, just use lon lat chunks  