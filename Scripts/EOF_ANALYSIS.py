...
def pca(data,coords,nmodes):
    #STDD=(data-np.nanmean(data))/np.std(data) #standardised anomalies 
    
    #working on the weights 
    coslat = np.cos(np.deg2rad(data.coords[coords].values)) 
    ...
    #calculate the EOFs 
    eof1=solver.eofsAsCovariance(neofs=nmodes)
    pc1 = solver.pcs(npcs=nmodes,pcscaling=1)
    eigenv2 = solver.eigenvalues(neigs=nmodes)
    variance_fractions = solver.varianceFraction(neigs=nmodes)
    return pc1,eof1,variance_fractions
...
path = f'/global/project/projectdirs/m1517/cascade/kquagra/temp_data_comp/'
otpath='/new_new_algo/'
...
#####################################################################################################################################################
#IVT
path = f'/global/project/projectdirs/m1517/cascade/kquagra/IVT_AR_capture_2/'
vbl_nm = 'IVT'
...
for f in flds:
    ...
    ds=xr.open_dataset(f'{path}{f}_{vbl_nm}.nc')[vbl_nm]#.sel(level=levels)
    stddiv = ((ds) - np.nanmean(ds))/ (np.nanstd(ds) * 6)
    ivt_col[f].append(stddiv.fillna(np.nanmean(stddiv)))    
print(f'Done Loading {vbl_nm}')
    
#GEOPTH
path = '/global/homes/k/kquagra/kquagra/EOF_STDs'
vbl_nm = 'Z'
fpath = '_GEOPTH'
...
for f in flds:
    ...
    gpt_col[f].append(xr.open_dataset(f'{path}{fpath}/std_eof_data{fpath}_{f}.nc')[vbl_nm])
print(f'Done Loading {vbl_nm}')
...
for f in flds:
    ...
    pv_col[f].append(xr.open_dataset(f'{path}{fpath}/std_eof_data{fpath}_{f}.nc')[vbl_nm])
print(f'Done Loading {vbl_nm}')
...
for f in flds:
    ...
    msl_col[f].append(xr.open_dataset(f'{path}{fpath}/std_eof_data{fpath}_{f}.nc')[vbl_nm])
print(f'Done Loading {vbl_nm}')
...
for f in flds:
    ...
    tcwv_col[f].append(xr.open_dataset(f'{path}{fpath}/std_eof_data{fpath}_{f}.nc')[vbl_nm])
...
for f in flds:
    ...
    temp_col[f].append(xr.open_dataset(f'{path}{fpath}/std_eof_data{fpath}_{f}.nc')[vbl_nm])
print(f'Done Loading {vbl_nm}')
...
data_eof = collections.defaultdict(list)
data_pc = collections.defaultdict(list)
data_var = collections.defaultdict(list)
...
for fx, f in enumerate(flds):
    ...
    data_pc[f],data_eof[f], data_var[f] = pca(dd,'latitude',4)  #calculate EOFs 
    ...
    data_eof[f].to_netcdf(f'{di1}EOF_output_{f}.nc')
    data_eof[f].to_netcdf(f'{di2}PC_output_{f}.nc')
    data_eof[f].to_netcdf(f'{di3}VAR_output_{f}.nc')
...
svb_nms = ['ivt','tcwv','mslp','geopth','temp','pv']
...
for fx, f in enumerate(flds):
    ...
    dirs = f'Images/new_levels_geopth_mean/M{mode_n+1}L{v_levs[lev]}/'
