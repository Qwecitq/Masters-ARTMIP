...
data_path = sys.argv[1]   #where find  the AR data "m1517/cascade/external_datasets/ARTMIP/tier1/ftp_mirror/"
save_path = sys.argv[2] #where to store the data that is computed "larger_region_AR_capture"
th1 = float(sys.argv[3]) ; th2 = float(sys.argv[4]) #thresholds 1 for the main box and 2 for the secondary boxes C and B #0.8 0.1
...
def reg_criteria_sel(data_path,save_path,th1,th2,llon,rlon,blat,tlat):
    ...
    for idx, dx in enumerate(fpath):
        ...
        for ff in file:
            ...
            with xr.open_dataset(ff)['ar_binary_tag'].sel(lon = slice(-115,-50), lat = slice(-10,-60)) as dd : 
                data = dd.isel(time=dd.time.dt.month.isin([6,7,8])) #laod data and select DJF season only for NH and JJA for SH
